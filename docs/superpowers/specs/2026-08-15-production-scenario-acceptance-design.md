...
商品图片来自明确采用 CC0-1.0 许可的 Clothing Dataset Small 固定提交；商品事实、供应商合规字段、变体、冲突和缺失事实由本地夹具确定性生成。这套验收不能被描述为真实企业数据或真实平台发布测试。它验证的是公开真实图片与接近生产资料的完整数据链路；真实 Shopify/eBay 测试店导入仍属于外部验收。
...
公开数据快照固定到 `alexeygrigorev/clothing-dataset-small` 的提交 `11961516512414bf9f6c150a832cfd21b54a6b54`，本地记录 CC0-1.0 许可、来源 URL 和 SHA-256，自动测试不访问网络。
