...
export interface HealthStatus { status: string; app_id: string; app_version: string; protocol_name: 'agentteams' | string; protocol_version: number; agentteams?: { status: string; ready: boolean; last_error?: string } }
...
export interface Artifact { id: string; project_id: string; worker_name: string; process_task_id: string; artifact_type: string; title: string; file_name: string; relative_path: string; mime_type: string; size_bytes: number; sha256: string; dependency_ids?: string[]; metadata: Record<string, unknown>; created_at: string }
...
export const api = {
  ...
  agentTeamsMessages: (taskId: string) => request<{ items: Array<Record<string, unknown>> }>(`/api/tasks/${encodeURIComponent(taskId)}/agentteams/messages`),
