...
class CatalogWorkflow:
    """Compatibility facade over the real external AgentTeams service.

    This class no longer plans or executes Workers. AgentTeams Manager and
    TeamHarness own that lifecycle. The only local behavior retained here is
    deterministic approval materialization before a new request is submitted.
    """
    ...
    def run_task(self, task_id: str) -> dict[str, Any]:
        ...
        delegation = self.app.agentteams_service.submit(
            task_id,
            project_id=str(task["project_id"]),
            objective=str(task["objective"]),
            context=self.app.task_contexts.snapshot_for_task(task_id),
        )
    ...
    def approve_and_rerun(self, approval_id: str, decision: dict[str, Any]) -> dict[str, Any]:
        ...
        if previous_materials:
            self.app.materials.bind_task(rerun["id"], previous["project_id"], [item["id"] for item in previous_materials])
        resolved_ids = self._apply_approval_decision(rerun, approval, decision)
        input_data.update({
            "selected_resource_ids": list(dict.fromkeys([*(str(item) for item in decision.get("resource_ids") or []), *resolved_ids])),
            "material_ids": [item["id"] for item in previous_materials],
            "source_paths": self.app.materials.task_paths(rerun["id"]),
        })
        self.app.tasks.update_input(rerun["id"], input_data)
        return self.run_task(rerun["id"])

    def _apply_approval_decision(self, rerun: dict[str, Any], approval: dict[str, Any], decision: dict[str, Any]) -> list[str]:
        ...
        if approval["approval_type"] == "catalog_conflict":
            ...
            facts.append(ProductFact(
                ...
                evidence=SourceEvidence(source_document_id=f"human_approval:{approval['id']}", file_name="人工审批", location=approval["id"], text=str(selected)),
            ).model_dump())
        raw["facts"] = facts
        product = CanonicalProduct.model_validate(raw)
        self.app.graph.upsert_candidate_graph([product], task_id=rerun["id"], project_id=rerun["project_id"])
        artifact = self.app.artifacts.create_text(
            rerun["id"], "catalog_steward_agent", "canonical_product", "canonical_product_approved",
            content=json_dumps({"products": [product.model_dump()]}), extension="json", mime_type="application/json",
            metadata={"logical_key": "canonical_product", "resource_status": "active"},
        )
        collection = self.app.resources.create(
            project_id=rerun["project_id"], resource_type="product_collection", logical_key="canonical-products",
            owner_worker_name="catalog_steward_agent", source_task_id=rerun["id"], storage_kind="database",
            storage_ref=rerun["project_id"], status="active", metadata={"product_ids": [product.id], "approval_id": approval["id"]},
        )
